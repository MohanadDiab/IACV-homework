...
%% See the effect of different edge detection techniques implemented in matlab
figure();
subplot(2, 4, 1),
imshow(img);
...
J1 = edge(preprocessed_im, 'Sobel');
subplot(2, 4, 2),
imshow(J1);
title("Sobel");

% Prewitt Edge detection
K1 = edge(preprocessed_im, 'Prewitt');
subplot(2, 4, 3),
imshow(K1);
title("Prewitt");

% Robert Edge Detection
L1 = edge(preprocessed_im, 'Roberts');
subplot(2, 4, 4),
imshow(L1);
title("Robert");

% Log Edge Detection
M1 = edge(preprocessed_im, 'log');
subplot(2, 4, 5),
imshow(M1);
title("Log");

% Zerocross Edge Detection
N1 = edge(preprocessed_im, 'zerocross');
subplot(2, 4, 6),
imshow(N1);
title("Zerocross");

% Canny Edge Detection
O1 = edge(preprocessed_im, 'Canny');
subplot(2, 4, 7),
imshow(O1);
title("Canny");

% we can see that Canny offers the best contours for our region of interest
% (the cone)
...
legend('L2','line3','L1','line4', 'Major-axis 1','Major-axis 2', 'infinity');
hold off;
...
f = real(double(res.f)); f = f(f > 0); f = f1;
